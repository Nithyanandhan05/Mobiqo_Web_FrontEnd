        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.status === 'success') {
                    ...
                    // Ensure real database products with actual prices and photos appear at the top
                    fetched.sort((a: SearchResult, b: SearchResult) => {
                        if (a.match_percent === 'In Stock' && b.match_percent !== 'In Stock') return -1;
                        if (a.match_percent !== 'In Stock' && b.match_percent === 'In Stock') return 1;
                        return 0;
                    });
                    setResults(fetched);
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
                ...
                <div className="relative mb-6">
                    ...
                    <input
                        ...
                        placeholder="Search smartphones... e.g. Samsung S24"
                        ...
                        {loading ? (
                            ...
                            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                {results.map((result, index) => {
                                    ...
                                    return (
                                        <div key={index} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-all page-enter" style={{ animationDelay: `${index * 80}ms` }}>
                                            <div className="flex items-start gap-4 mb-4">
                                                <div className="w-20 h-20 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-center shrink-0 p-2">
                                                    <img src={getHDImage(result.image_url, result.name)} alt={result.name} className="w-full h-full object-contain" onError={(e) => { (e.target as HTMLImageElement).src = 'https://fdn2.gsmarena.com/vv/bigpic/samsung-galaxy-s24-ultra-5g-sm-s928-u1.jpg'; }} />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span className="px-2 py-0.5 bg-blue-50 text-primary text-[10px] font-black uppercase tracking-wider rounded">Live Web</span>
                                                        <div className="flex items-center gap-0.5 ml-auto">
                                                            <span className="material-symbols-outlined text-amber-400 text-sm font-variation-fill">star</span>
                                                            <span className="text-xs font-bold text-slate-600">{(4.5 + Math.random() * 0.5).toFixed(1)}</span>
                                                        </div>
                                                    </div>
                ...
                {activeQuery.length < 2 && (
                    <div className="text-center py-16">
                        ...
                        <p className="text-slate-500 font-medium max-w-md mx-auto">Type a smartphone name above to search from live web sources. Add 2 devices to compare their specifications side by side.</p>
                    </div>
                )}
            </div>
                            ...
                            {selectedDevices.map((device, i) => (
                                <div key={i} className="flex items-center gap-3 bg-slate-50 rounded-2xl px-4 py-2.5 border border-slate-100 relative">
                                    ...
                                    <div className="w-10 h-10 bg-white rounded-xl border border-slate-100 flex items-center justify-center p-1 shrink-0">
                                        <img src={getHDImage(device.image_url, device.name)} alt={device.name} className="w-full h-full object-contain" onError={(e) => { (e.target as HTMLImageElement).src = 'https://fdn2.gsmarena.com/vv/bigpic/samsung-galaxy-s24-ultra-5g-sm-s928-u1.jpg'; }} />
                                    </div>
