        ...
        <AdminLayout activeTab="dashboard" onNavigate={onNavigate}>
            <div className="flex items-center justify-between mb-10">
                <div>
                    ...
                    <p className="text-sm font-bold text-slate-500 mt-1 uppercase tracking-widest">Global operations and warranty surveillance dashboard.</p>
                </div>
                <div className="flex items-center gap-4">
                    <button className="px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-black text-slate-700 hover:bg-slate-50 transition-all flex items-center gap-2 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
                {statCards.map((stat, i) => (
                    <div key={i} className="group bg-white p-6 rounded-3xl border border-slate-200 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5 transition-all relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className={`w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-primary/10 group-hover:text-primary transition-colors`}>
                                <span className="material-symbols-outlined text-xl font-variation-fill">{stat.icon}</span>
                            </div>
                        </div>
                        <div className="flex items-end gap-3 text-3xl font-black text-slate-900 leading-none">
                            {stat.value}
                        </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-2 bg-white rounded-[32px] border border-slate-200 overflow-hidden shadow-sm">
                    <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        <h2 className="text-xl font-black text-slate-900">Recent Warranty Claims</h2>
                                    ...
                                    <tr key={idx} className="hover:bg-slate-50/50 transition-colors group cursor-pointer">
                                        <td className="px-8 py-5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-sm font-bold text-slate-700">{claim.user_name}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5 text-sm font-bold text-slate-600">{claim.device}</td>
                                        <td className="px-6 py-5">
                                            <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${claim.status === 'Approved' ? 'bg-emerald-100 text-emerald-700' :
                                        ...
                                        <td className="px-6 py-5 text-sm font-bold text-slate-500">{claim.date}</td>
                                        <td className="px-8 py-5">
                ...
                <div className="space-y-8 flex flex-col">
                    ...
                    <div className="bg-primary rounded-[32px] p-8 text-white relative overflow-hidden shadow-xl shadow-primary/20 flex-1 flex flex-col items-center justify-center text-center">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl -mr-20 -mt-20"></div>
                        <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center text-white mb-8 border border-white/30 backdrop-blur-sm self-center">
                            <span className="material-symbols-outlined font-variation-fill text-3xl">psychology</span>
                        </div>
                        <h3 className="text-xl font-black mb-10">AI Analytics Insight</h3>

                        <div className="relative w-48 h-48 mx-auto mb-10">
                            <svg className="w-full h-full transform -rotate-90">
                                <circle cx="96" cy="96" r="88" strokeWidth="12" stroke="currentColor" fill="transparent" className="text-white/20" />
                            ...
                            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                                <div className="text-4xl font-black leading-none">75%</div>
                                <div className="text-[10px] font-black uppercase tracking-widest mt-1 opacity-60">Mobile</div>
                            </div>
                        </div>

                        <p className="text-sm font-medium text-white/80 leading-relaxed max-w-[280px] mx-auto">
                    ...
                    <div className="bg-white rounded-[32px] p-8 border border-slate-200 shadow-sm">
                        <div className="flex items-center gap-2 mb-8">
                            ...
                            <h2 className="text-lg font-black text-slate-900">Top Trending Queries</h2>
                        </div>
                        <div className="space-y-6">
                            {trendingQueries.map((q, i) => (
                                <div key={i} className="flex items-center justify-between">
                                    <span className="text-sm font-bold text-slate-600">{q.query}</span>
                                    <span className="bg-slate-100 text-primary px-3 py-1 rounded-lg text-[11px] font-black tracking-wider shadow-sm border border-white">
                                        {q.count}
                                    </span>
                                </div>
                            ))}
